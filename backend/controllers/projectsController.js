...
class ProjectsController{
    static async createItem(req,res){
        ...
        try{
            await query('INSERT INTO projects (name,description,due_date,priority,status,notes,created_at,user_id) VALUES(?,?,?,?,?,?,?,?)',[name,description,due_date,priority,status,notes,created_at,req.user.userId]);
    ...
    static async getItem(req,res){
        ...
        try{
            const item = await query('SELECT * FROM projects WHERE id = ? AND user_id = ?',[itemId,req.user.userId]);
            if(item.length === 0) return res.status(404).json({message: "Project not found!"});
    ...
    static async getItems(req,res){
        try {
            const items = await query('SELECT * FROM projects WHERE user_id = ?',[req.user.userId]);
